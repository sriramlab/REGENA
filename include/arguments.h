...
struct options{
	...
	bool   use_boxcox;       // --use-boxcox
	double boxcox_lambda_min; // --boxcox-min
	double boxcox_lambda_max; // --boxcox-max
	int    boxcox_n_points;   // --boxcox-n
};
...
std::string usage ( ) {
	...
	ostr <<"Usage: REGENA " << "[--config parameter file|command-line options]" << endl;
	ostr << "\nOptions:\n"
		 ...
		 << "  -eXa, --eXannot				By default, REGENA fits a single GxE variance component. To partition the GxE component w.r.t the annotation file, add '-eXannot' flag.\n"
		 ...
		 << "  -bx, --use-boxcox				Fit the model on a grid of Box-Cox transformed phenotypes (requires a binary environment and a strictly positive phenotype).\n"
		 << "  -bx-min, --boxcox-min				Smallest Box-Cox lambda in the grid (default: -2).\n"
		 << "  -bx-max, --boxcox-max				Largest Box-Cox lambda in the grid (default: 2).\n"
		 << "  -bx-n, --boxcox-n				Number of evenly spaced lambda values in the grid (default: 100).\n"
		 << endl;
	return ostr.str ();
}
...
void parse_args(int argc, char const *argv[]){
	...
	command_line_opts.use_boxcox = false;
	command_line_opts.boxcox_lambda_min = -2;
	command_line_opts.boxcox_lambda_max = 2;
	command_line_opts.boxcox_n_points = 100;
	...
	cfg.insertKey ("use-boxcox", "0");
	cfg.insertKey ("boxcox-min", "-2");
	cfg.insertKey ("boxcox-max", "2");
	cfg.insertKey ("boxcox-n", "100");
...
//	cfg.insertKey ("gene_by_env", "1");
	...
	if (strcmp(argv[1],"--config")==0) {
		...
		command_line_opts.use_boxcox = cfg.getValueOfKey<bool>("use-boxcox", false);
		command_line_opts.boxcox_lambda_min = cfg.getValueOfKey<double>("boxcox-min", -2);
		command_line_opts.boxcox_lambda_max = cfg.getValueOfKey<double>("boxcox-max", 2);
		command_line_opts.boxcox_n_points = cfg.getValueOfKey<int>("boxcox-n", 100);
		...
		for (int i = 1; i < argc; i++) {
			if (i + 1 != argc){
				if(isarg(argv[i], "-g", "--genotype")){
					...
					i++;
                } else if (isarg(argv[i],"-bx", "--use-boxcox") ) {
                    command_line_opts.use_boxcox = true;     
                    cfg.insertKey ("use-boxcox", "1", 0);
                } else if (isarg(argv[i],"-bx-min", "--boxcox-min") ) {
                    command_line_opts.boxcox_lambda_min = atof(argv[i+1]);
                    cfg.insertKey ("boxcox-min", argv[i+1], 0);
					i++;
                } else if (isarg(argv[i],"-bx-max", "--boxcox-max") ) {
                    command_line_opts.boxcox_lambda_max = atof(argv[i+1]);     
                    cfg.insertKey ("boxcox-max", argv[i+1], 0);
					i++;
                } else if (isarg(argv[i],"-bx-n", "--boxcox-n") ) {
                    command_line_opts.boxcox_n_points = atoi(argv[i+1]);     
                    cfg.insertKey ("boxcox-n", argv[i+1], 0);
					i++;
                } else {
					cerr << version_string << endl;
